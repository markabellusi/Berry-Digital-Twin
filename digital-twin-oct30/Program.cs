namespace BerryTwinProducerConsumerModel
{
    ...
    class Program
    {
        private static CancellationTokenSource tokenSource; // Declare a static CancellationTokenSource variable.

        static async Task Main(string[] args)
        {
            string logDirectory = "logs"; // Define the name of the log directory.
            EnsureLogDirectoryExists(logDirectory); // Call a method to ensure that the log directory exists.

            await RunHopperBlenderAndExtruder(); // Call an asynchronous method to run some operations.

            Logger.Log("done!"); // Log a message indicating the completion of the operations.
        ...
        {
            if (!Directory.Exists(logDirectory)) // Check if the specified log directory doesn't exist.
            {
        ...
        private static async Task HandleErrorsAsync(Func<Task> taskFunction, string componentName)
        {
            try
            {
                await taskFunction.Invoke(); // Invoke the provided asynchronous task function.
            }
            catch (OperationCanceledException)
            {
                Logger.Log($"{componentName} was interrupted.", ConsoleColor.Magenta); // Log a message if the task was canceled.
            }
            catch (Exception ex)
            {
                Logger.LogError($"{componentName} error: {ex.Message}", ex); // Log an error message if an exception occurs during the task.
            }
        }
             ...
             private static async Task BlenderConsumerAsync(Channel<Envelope> channel, CancellationToken cancellationToken)
            {
                ...
                int consumedCount = 0;
                int maxConsumedCount = 10; // Change this to control how much the Blender consumes
            ...
            private static async Task ExtruderProducerAsync(Channel<Envelope> channel, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    // Static sequence of Extruder states.
                    ...
                    while (!cancellationToken.IsCancellationRequested)
                    {
                        ...
                        currentIndex = (currentIndex + 1) % extruderStates.Length; // Move to the next state in a loop.
                        // Simulate some work.
            ...
            private static async Task ExtruderConsumerAsync(Channel<Envelope> channel, CancellationToken cancellationToken)
            {
                ...
                bool errorOccurred = false;
                int consumedCount = 0;
                int maxConsumedCount = 10000; // Change this to control how much the Extruder consumes
